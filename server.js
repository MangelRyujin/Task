...
app.post("/api/chatbot", async (req, res) => {
  ...
  try {
    ...
    const isSpanish = lang?.startsWith("es"); // cualquier español cae aquí
    ...
    // 🔹 Clean response (in case it comes with line breaks or quotes)
    const task = data?.result?.response?.trim().replace(/^["']|["']$/g, "") || text;
